...
| Add item with identical composite primary key            | `errSecDuplicateItem` (-25299) | Requires `SecItemUpdate` duplicate handling       |
...
11. **Duplicate handling** — Always handle `errSecDuplicateItem` with an update-first flow or an add-then-update fallback; avoid delete-before-add unless you are intentionally migrating primary key attributes
