...
class DatabaseHelper {
    ...
    static let migrations: [SchemaMigration] = [
        ...
        SchemaMigration(toVersion: 75) { db in
            ...
            try db.executeUpdate(
                "CREATE INDEX file_sync_journal_entity ON FileSyncJournal (entityType, entityUuid, flushedSeq);", values: nil)
            try db.executeUpdate(
                "CREATE INDEX user_episode_folder_relative_path ON SJUserEpisode (folderRelativePath);", values: nil)
            try db.executeUpdate(
                "CREATE INDEX user_episode_content_hash ON SJUserEpisode (contentHash);", values: nil)
        }
    ]
