...
/// Turns pending journal entries into op envelopes and appends them to this
/// device's current log file. Values are read from the live DB rows at
/// flush time (the journal stores which fields changed, not their values),
/// so a burst of edits flushes as one up-to-date op.
struct OpJournalFlusher {
    ...
    func flush(settings: [FileSyncSettingChange], stats: Filesync_StatsCumulative?) async throws -> Result {
        ...
        var flushedEntryIDs: [Int64] = []
        var nextSeq = ownCursor.headSeq

        for group in entries {
            guard let payload = payload(for: group) else {
                // Unresolvable entries (rows since deleted, provisional
                // uploads) are skipped but marked flushed so they don't pile
                // up; provisional uploads re-journal at hash promotion.
                if group.holdBack == false {
                    flushedEntryIDs.append(contentsOf: group.entryIDs)
                }
                continue
            }
            nextSeq += 1
        ...
        // Settings/stats are pulled (not journaled); emit only on change,
        // tracked by a digest persisted alongside the cursor row.
        let settingsDigest = settings.map { "\($0.name)=\($0.jsonValue)@\($0.modifiedAtMs)" }.sorted().joined(separator: "|")
        let statsDigest = stats.map { "\($0.timeListened)|\($0.timesStartedAt)|\($0.timeSkipping)|\($0.timeIntroSkipping)|\($0.timeSilenceRemoval)|\($0.timeVariableSpeed)" } ?? ""
        ...
        if digest != previousDigest {
            ...
            if let stats {
                ...
                envelope.deviceID = deviceID
                envelope.seq = UInt64(nextSeq)
                envelope.wallClockMs = DBUtils.currentUTCTimeInMillis()
        ...
        // Append to the current log (single-writer directory: safe
        // read-modify-write), rotating when full.
        ...
        // Durably written: record flush state.
        let firstDataSeq = ownCursor.headSeq + 1
        dataManager.markFileSyncEntriesFlushed(entryIDs: flushedEntryIDs, startingSeq: firstDataSeq)
        ownCursor.headSeq = nextSeq
        dataManager.save(fileSyncCursor: ownCursor)
        UserDefaults.standard.set(digest, forKey: digestKey)
        let cutoff = DBUtils.currentUTCTimeInMillis() - Int64(7 * 24 * 60 * 60 * 1000)
    ...
    /// Upserts for the same entity merge their changed-field sets; ordered
    /// ops (up next, deletes) stay individual and ordered.
    func coalesce(_ entries: [FileSyncJournalEntry]) -> [EntryGroup] {
        ...
        for entry in entries {
            ...
            groups.append(EntryGroup(
                entityType: entity, opType: op, entityUuid: entry.entityUuid,
                changedFields: Set(entry.changedFieldNames), fields: entry.fields,
                wallClockMs: entry.wallClockMs, entryIDs: [entry.id]))
        }
        return groups
    }
    ...
    func payload(for group: EntryGroup) -> Filesync_OpEnvelope.OneOf_Payload? {
        ...
        case (.userEpisode, .upsert):
            ...
            guard episode.identity == .canonical,
    ...
    private func podcastUuid(forQueuedEpisode uuid: String) -> String {
        if let episode = dataManager.findBaseEpisode(uuid: uuid) {
            return episode.parentIdentifier()
        }
        return ""
    }
}
