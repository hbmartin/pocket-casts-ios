...
/// Writes this device's full-state checkpoint and compacts its own logs,
/// so new devices bootstrap fast and old history doesn't accumulate.
struct SnapshotWriter {
    let folder: any SyncFolder
    let dataManager: DataManager
    let deviceID: String

    /// Writes a snapshot when the un-snapshotted own logs exceed the size
    /// threshold or the newest snapshot is stale, then compacts covered
    /// logs older than the grace period. `fullState` is evaluated lazily
    /// (a complete re-read of all devices) only when a snapshot is due.
    func snapshotIfNeeded(
        headSeq: Int64, nowMs: Int64,
        ...
        // Compact: own logs fully covered by this snapshot and older than
        // the grace window; and superseded older snapshots.
        let graceCutoff = nowMs - Int64(FileSyncFormat.logCompactionGracePeriod * 1000)
        for log in logs where log.mtimeMs < graceCutoff {
            // The active log is the newest by name; never delete it.
            if log.fileName == logs.map(\.fileName).max() { continue }
