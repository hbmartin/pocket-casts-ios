...
/// DB model ↔ sync record conversions, shared by the flusher (outgoing),
/// applier (incoming), bootstrap seed, and snapshot writer.
///
/// Field maps mirror the server sync engine's proven conversions
/// (SyncTask+LocalChanges outgoing, SyncTask+ServerChanges incoming) so the
/// file format stays semantically identical to what the app already syncs.
/// Nested per-podcast settings (`Api_PodcastSettings`) are not yet mapped —
/// they ride with the server engine's settingsSync flag and are a follow-up.
enum RecordConverters {

    // MARK: - Outgoing (DB row -> record)
    ...
    /// Only the named changed fields ride in the record (plus their
    /// embedded *Modified stamps, which drive the LWW merge).
    ...
    // MARK: - Incoming (merged record -> DB row updates)

    /// Applies merged podcast fields onto an existing row. Returns the
    /// updated value; caller saves under suppression.
