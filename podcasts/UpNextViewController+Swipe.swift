    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath, for orientation: SwipeActionsOrientation) -> [SwipeAction]? {
        ...
        case .left:
            let moveToTopAction = SwipeAction(style: .default, title: nil) { [weak self] _, indexPath in
                guard let self, let episode = PlaybackManager.shared.episodeInUpNextAt(index: indexPath.row) else { return }

                Analytics.track(.episodeSwipeActionPerformed, properties: ["action": "up_next_move_up", "source": "up_next"])

                PlaybackManager.shared.moveUpNextEpisode( episode, to: 0, fireNotification: false)
            ...
            let moveToBottomAction = SwipeAction(style: .default, title: nil) { [weak self] _, indexPath in
                ...
                PlaybackManager.shared.moveUpNextEpisode( episode, to: queueCount - 1, fireNotification: false)
        ...
        case .right:
            let deleteAction = SwipeAction(style: .destructive, title: nil) { [weak self] _, indexPath in
                ...
                let remainingEpisodes = PlaybackManager.shared.upNextCount()
