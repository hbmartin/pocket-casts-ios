...
When adding strings add then with meaningful keys that describe `feature_` + `relevantIdentifier(s)_` + `description`. Keep keys under 255 characters — some translation platforms truncate longer keys, which can cause issues with detecting changes.
