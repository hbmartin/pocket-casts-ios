...
| `newSettingsStorage` | **true** (A6a, 2026-07-12) | 153 | AppDelegate+Defaults.swift, AutoAddQueueDataManager.swift, A | enabled — remote `new_settings_storage` key is a live kill switch; collapse the 153 call sites via A6b after one release of soak (docs/DeferredWork.md) | || `episodeSummaries` | conditional (`!= .appStore`) | 1 | EpisodeDetailViewController+ShowNotes.swift | keep (staged rollout — AI UX plan Phase 2: episode summary card + takeaways) | |
