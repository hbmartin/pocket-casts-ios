...
| 3 | Activity feed, now-playing presence, milestones, "recently played by" shelf, weekly digest | in-progress | **Slice 5 shipped the feed (2026-07-17):** fan-out-on-read derivation over existing tables (ADR-0009), six event kinds, per-field-visibility gated, mute/block filtered, on the Explore tab. **Slice 14 (2026-07-19, ADR-0013): milestones + weekly digest** — two global ladders (hours, episodes finished, tiers {10,50,100,250,500,1000}); crossings MATERIALIZED at sync-apply time (the crossing moment is state, not derivable — narrow ADR-0009 amendment); stats_visibility gates the feed kind + profile line, own crossings always celebrated locally (share via Slice-3 cards); digest = push type 9, on by default, guarded (joined AND (graph OR fresh milestone)), weekly asynq job, sent-watermark. Still open: presence TTL + ops cost; shelf. | sync events; push token infra; `NotificationsCoordinator` |
