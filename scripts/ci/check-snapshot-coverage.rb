...
class SnapshotCoverageCheck
  ...
  FileCoverage = Struct.new(:name, :path, :covered_lines, :executable_lines, keyword_init: true) do
  ...
  def run
    ...
    groups = @options.fetch(:groups).map { |name, identifiers| coverage_for_group(name, identifiers, files) }
    failures = failures_for(groups)
  ...
  rescue OptionParser::ParseError, JSON::ParserError, KeyError, RuntimeError => e
  ...
  def coverage_files(report)
    ...
    report.fetch('targets').each do |target|
      target.fetch('files').each do |file|
        ...
        files_by_path[path] ||= FileCoverage.new(
  ...
  def failures_for(groups)
    groups.flat_map do |group|
      ...
      unless group.missing_files.empty?
        group_failures << "#{group.name} is missing required file(s): #{group.missing_files.join(', ')}"
      end
      ...
      unless group.zero_executable_files.empty?
        group_failures << "#{group.name} has no executable coverage lines for: #{group.zero_executable_files.join(', ')}"
      end

      if group.coverage_percent < threshold
        group_failures << "#{group.name} coverage is #{percent(group.coverage_percent)}, below #{percent(threshold)}"
      end
  ...
  def print_summary(report, groups)
    ...
    groups.each do |group|
      @stdout.puts "  #{group.name}: #{percent(group.coverage_percent)} (#{group.covered_lines}/#{group.executable_lines})"
    end
    @stdout.puts "Overall linked target coverage: #{percent(overall_percent(report))} " \
                 "(#{report.fetch('coveredLines')}/#{report.fetch('executableLines')}) [reported only]"
  end

  def overall_percent(report)
