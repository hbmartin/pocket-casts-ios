...
final class NarrationQueueTests: DBTestCase {
    ...
    private var createdNarrationUuids: [String] = []
    ...
    override func tearDown() async throws {
        ...
        // go back out the way they came in.
        for uuid in createdNarrationUuids {
            dataManager.narrations.delete(uuid: uuid)
        }
        createdNarrationUuids = []
    ...
    private func makeNarration(text: String = NarrationQueueTests.sourceText) throws -> NarrationRecord {
        ...
        let sourcePath = try storage.writeSource(text: text, narrationUuid: uuid)

        var record = NarrationRecord()
        record.uuid = uuid
        record.title = "Test Document"
        record.sourcePath = sourcePath
        record.characterCount = Int32(text.count)
        ...
        createdNarrationUuids.append(uuid)
        return record
    }
    ...
    func testRenderingProducesAnEpisodeAndMarksCompleted() async throws {
        ...
        let loaded = try XCTUnwrap(dataManager.narrations.narration(uuid: narration.uuid))
    ...
    func testTheSourceDocumentSurvivesAnyRender() async throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: storage.sourceURL(relativePath: narration.sourcePath).path))
    }
    ...
    func testResumeSkipsChunksAlreadyOnDisk() async throws {
        ...
        let afterCrash = try XCTUnwrap(dataManager.narrations.narration(uuid: narration.uuid))
        ...
        XCTAssertEqual(dataManager.narrations.narration(uuid: narration.uuid)?.narrationState, .completed)
    }
    ...
    func testResumeTrustsTheFilesystemOverAStaleCount() async throws {
        ...
        dataManager.narrations.updateProgress(uuid: narration.uuid, completedChunkCount: 0)
        ...
        XCTAssertEqual(dataManager.narrations.narration(uuid: narration.uuid)?.completedChunkCount, Int32(renderedCount))
    }

    func testRestorePendingPicksUpAnInterruptedRender() async throws {
        let narration = try makeNarration()
        dataManager.narrations.markRendering(uuid: narration.uuid, chunkCount: 3)
    ...
    func testCompletedNarrationsAreNotResumed() async throws {
        let narration = try makeNarration()
        dataManager.narrations.markCompleted(uuid: narration.uuid, episodeUuid: "ep-1", duration: 10, sizeInBytes: 10)
    ...
    func testCancellationClearsTheWorkspace() async throws {
        ...
        XCTAssertEqual(dataManager.narrations.narration(uuid: narration.uuid)?.narrationState, .cancelled)
        XCTAssertFalse(FileManager.default.fileExists(atPath: storage.workspaceURL(narrationUuid: narration.uuid).path))
    }
    ...
    func testAMissingSourceFileFailsWithSourceUnreadable() async throws {
        ...
        XCTAssertEqual(
            dataManager.narrations.narration(uuid: narration.uuid)?.errorCode,
    ...
    func testNarrationsRenderOneAtATime() async throws {
        ...
        XCTAssertEqual(dataManager.narrations.narration(uuid: first.uuid)?.narrationState, .completed)
        XCTAssertEqual(dataManager.narrations.narration(uuid: second.uuid)?.narrationState, .completed)
